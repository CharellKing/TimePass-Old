...
#include "shm/shm_multihashmap.h"
...
struct MonthName {
    ...
    static off_t HashFunc(const MonthName& month_name) {
        return HashFunc(month_name.t_month);
    }
    char t_month[32];
};
...
void ToDotPs(const char* name, const TimePass::ShmMultihashmap<MonthName, int>* p_l) {
	char cmd[100];
	char file[100];
...
void Create(off_t len) {
	TimePass::ShmMultihashmap<MonthName, int> months("/tmp/hashmap");
