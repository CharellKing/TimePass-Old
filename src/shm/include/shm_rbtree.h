#ifndef SHM_RBTREE_H_

#define SHM_RBTREE_H_
...
template <typename KEY, typename VALUE, int (*Comp)(const KEY& a, const KEY& b)=KEY::Compare>
struct ShmPair{
    ShmPair(KEY key = KEY(), VALUE value = VALUE()):first(key), second(value) {
        
    }
    static int Compare (const ShmPair<KEY, VALUE, Comp>& a, const ShmPair<KEY, VALUE, Comp>& b) {
...
struct RbtreeHead{
    RbtreeHead(off_t root, off_t capacity, off_t size, off_t free_stack):root(root), capacity(capacity), size(size), free_stack(free_stack) {
...
public:
    //唯一key插入
    static bool InsertUnique(const char* name, RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data, bool is_expand = true) {
        char which_child = 'L';
        off_t parent = FindUniqueInsertPoint(p_head, p_addr, data, which_child);
        //存在同一key值，就进行值的替换
        if ('M' == which_child) {
            (p_addr + parent)->data = data;
        } else {
            return Insert(name, p_head, p_addr, parent, which_child, data, is_expand);
        }
        return true;
    }
    
    //允许多key插入
    static bool InsertMultiple(const char* name, RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data, bool is_expand = true) {
        char which_child = 'L';
        off_t parent = ShmRbtree<T, Compare, EXTEND>::FindMultipleInsertPoint(p_head, p_addr, data, which_child);
        return ShmRbtree<T, Compare, EXTEND>::Insert(name, p_head, p_addr, parent, which_child, data, is_expand);
    }
    
    //删除元素
    static off_t Remove(RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data) {
    ...
    static off_t RemoveAll(RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data) {
        off_t count = 0;
        off_t target = FindNode(p_head, p_addr, data);
        while (-1 != target) {
    ...
    //升序遍历红黑树， 起始节点
    static RbtreeNode<T>* Begin(const RbtreeHead* p_head, RbtreeNode<T>* p_addr) {
        return AT(p_addr, Minimum(p_addr, p_head->root));
    }
    
    //升序遍历红黑树， 下一个节点
    static RbtreeNode<T>* Next(RbtreeNode<T>* p_addr, const RbtreeNode<T>* p_cur) {
        if (NULL == p_cur) {
            return NULL;
        }
    ...
    //升序遍历红黑树， 起始节点
    static const RbtreeNode<T>* Begin(const RbtreeHead* p_head, const RbtreeNode<T>* p_addr) {
        return AT(p_addr, Minimum(p_addr, p_head->root));
    }
    
    //升序遍历红黑树， 下一个节点
    static const RbtreeNode<T>* Next(const RbtreeNode<T>* p_addr, const RbtreeNode<T>* p_cur) {
        if (NULL == p_cur) {
            return NULL;
        }
        const RbtreeNode<T>*  p_next = AT(p_addr, Minimum(p_addr, p_cur->right));
        if (NULL == p_next) {
            const RbtreeNode<T>* p_child = p_cur;
    ...
    //降序遍历红黑树， 起始节点
    static RbtreeNode<T>* RBegin(const RbtreeHead* p_head, RbtreeNode<T>* p_addr) {
        return AT(p_addr, Maximum(p_addr, p_head->root));
    }
    
    //降序遍历红黑树， 下一个节点
    static RbtreeNode<T>* RNext(RbtreeNode<T>* p_addr, const RbtreeNode<T>* p_cur) {
        if (NULL == p_cur) {
            return NULL;
        }
    ...
    //降序遍历红黑树， 起始节点
    static const RbtreeNode<T>* RBegin(const RbtreeHead* p_head, const RbtreeNode<T>* p_addr) {
        return AT(p_addr, Maximum(p_addr, p_head->root));
    }
    
    //升序遍历红黑树， 下一个节点
    static const RbtreeNode<T>* RNext(const RbtreeNode<T>* p_addr, const RbtreeNode<T>* p_cur) {
        if (NULL == p_cur) {
            return NULL;
        }
    ...
    //<data的上限
    static off_t LowerBound(const RbtreeNode<T>* p_addr, off_t root, const T& data) {
        off_t res = -1;
        if(-1 != root){
            const RbtreeNode<T>* p_root = p_addr + root;
            off_t res = -1;
    ...
    //>data的下限
    static off_t UpperBound(const RbtreeNode<T>* p_addr, off_t root, const T& data) {
        off_t res = -1;
        if (-1 != root) {
            const RbtreeNode<T>* p_root = p_addr + root;
            int comp = Compare(data, p_root->data);
    ...
    //等于的范围
    static off_t EqualRange(const RbtreeNode<T>* p_addr, off_t root, const T& data) {
        off_t res = -1;
        if (-1 != root){
            const RbtreeNode<T>* p_root = p_addr + root;
            int comp = Compare(data, p_root->data);
    ...
    static bool Optimize(const char* name, RbtreeNode<T>* p_addr, RbtreeHead* p_head) {
        ...
        OptimizeTraverseMove(p_addr, p_head, -1, 'M', p_head->root, new_capacity);

        //过滤掉回收站中的[new_capacity, --)
        FliterFreeStack(p_addr, p_head, new_capacity);
        
        //开始缩减
        off_t len = sizeof(RbtreeHead) + sizeof(EXTEND) + sizeof(RbtreeNode<T>) * new_capacity;
...
private:
    static void OptimizeTraverseMove(RbtreeNode<T>* p_addr, RbtreeHead* p_head, off_t parent, char which_child, off_t src, off_t new_capacity) {
        if (src >= 0) {
            if (src >= new_capacity) {
            ...
            OptimizeTraverseMove(p_addr, p_head, parent, 'L', (p_addr + src)->left, new_capacity);
            OptimizeTraverseMove(p_addr, p_head, parent, 'R', (p_addr + src)->right, new_capacity);
        }
    }
    
    static void Move(RbtreeNode<T>* p_addr, off_t parent, char which_child, off_t src, off_t dest) {
    ...
    static off_t FliterFreeNode(RbtreeNode<T>* p_addr, RbtreeHead* p_head, off_t upper_limit) {
        off_t ret = -1; 
    ...
    static void FliterFreeStack(RbtreeNode<T>* p_addr, RbtreeHead* p_head,  off_t upper_limit) {
    ...
    static off_t FindUniqueInsertPoint(const RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data, char& which_child) {
        RbtreeNode<T>* p_parent = AT(p_addr, p_head->root);
        off_t parent = p_head->root;
        RbtreeNode<T>* p_child = p_parent;
        int comp = 0;
        which_child = 'L';//如果为空树，为M会有问题
        while(p_child) {
            p_parent = p_child;
    ...
    static off_t FindMultipleInsertPoint(const RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data, char& which_child) {
        RbtreeNode<T>* p_parent = AT(p_addr, p_head->root);
        int comp = 0;        
        off_t parent = p_head->root;
        which_child = 'L';
    ...
    //插入元素
    static bool Insert(const char* name, RbtreeHead* p_head, RbtreeNode<T>* p_addr, off_t parent, char which_child, const T& data, bool is_expand) {
        //获取新节点
        off_t freenode = -1;
        RbtreeNode<T>* p_freenode = GetFreeNode(name, p_head, p_addr, freenode, is_expand);
        ...
        if (-1 == parent) {
            ...
            if ('L' == which_child) {//新节点插入到父节点的左边
                p_parent->left = freenode;
            } else { //新节点插入到父结点的右
                p_parent->right = freenode;
            }
            if ('R' == p_parent->color) {//插入节点的父节点为红色则要修正红黑树，使其平衡
    ...
    //寻找KEY为T的节点
    static off_t FindNode(const RbtreeHead* p_head, RbtreeNode<T>* p_addr, const T& data) {
        int comp = 0;
    ...
    //删除节点
    static off_t RemoveNode(RbtreeHead* p_head, RbtreeNode<T>* p_addr, off_t target) {
        char which_child = 'L';
        off_t remove = -1, child = -1, parent = -1;
        RbtreeNode<T>* p_remove = NULL;
        RbtreeNode<T>* p_child = NULL;
        RbtreeNode<T>* p_ret = NULL;
        ...
        } else { //对于没有空子节点的节点，需要先找到一个叶子节点的值来替换删除结点的值，然后删除此叶子节点
            remove = Successor(p_addr, p_target);
            p_remove = AT(p_addr, remove);
            p_target->data = p_remove->data; //将要删除的节点的值copy给目标节点
    ...
    static off_t Successor(const RbtreeNode<T>* p_addr, const RbtreeNode<T>* p_node) {
    ...
    static void RemoveFixUp(RbtreeHead* p_head, RbtreeNode<T>* p_addr, off_t parent, off_t child) {
        RbtreeNode<T>* p_child = AT(p_addr, child);
        RbtreeNode<T>* p_parent = AT(p_addr, parent);
        while(p_head->root != child && 'B' == COLOR(p_child)) {
            if (child == p_parent->left) {//1：x节点为父节点的左孩子
                ...
                if ('R' == COLOR(p_sibling)) {//case 1：兄弟为红色
                    ...
                    RbtreeNode<T>* p_left_nephew = AT(p_addr, p_sibling->left);
                    RbtreeNode<T>* p_right_nephew = AT(p_addr, p_sibling->right);
                    
                    if ('B' == COLOR(p_left_nephew) && 'B' == COLOR(p_right_nephew)) {//case 2:x的左侄子和右侄子都为黑色
                        p_sibling->color = 'R';
                        p_child = p_parent;
                        child = parent;
                        p_parent = AT(p_addr, p_child->parent);
                        parent = p_child->parent;
                    } else if ('R' == COLOR(p_left_nephew) && 'B' == COLOR(p_right_nephew)){//case 3:x的左侄子为红色，右侄子为黑色
                        p_left_nephew->color = 'B';
                        p_sibling->color = 'R';
                        RightRotate(p_head, p_addr, sibling);
                    } else if ('R' == COLOR(p_right_nephew)) {//case 4： x的右侄子为红色
                ...
                if ('R' == COLOR(p_sibling)) {//case1：兄弟节点为红色
                    ...
                    RbtreeNode<T>* p_left_nephew = AT(p_addr, p_sibling->left);
                    RbtreeNode<T>* p_right_nephew = AT(p_addr, p_sibling->right);
                    if ('B' == COLOR(p_left_nephew) && 'B' == COLOR(p_right_nephew)) {//case 2：x的左侄子和右侄子都是黑色
                        p_sibling->color = 'R';
                        p_child = p_parent;
                        child = parent;
                        p_parent = AT(p_addr, p_child->parent);
                        parent = p_child->parent;
                    } else if ('R' == COLOR(p_right_nephew) && 'B' == COLOR(p_left_nephew)) {//case 3: x的右侄子为红色，左侄子为黑色
                        p_sibling->color = 'R';
                        p_right_nephew->color = 'B';
                        LeftRotate(p_head, p_addr, sibling);
                    } else if ('R' == COLOR(p_left_nephew)) {//case 4： x的左侄子为红色
                        p_sibling->color = p_parent->color;
                        p_parent->color = 'B';
                        p_left_nephew->color = 'B';
                        RightRotate(p_head, p_addr, parent);
    ...
    //连接两个节点
    static void Connect(RbtreeNode<T>* p_addr, off_t parent, off_t child, char which_child) {
        RbtreeNode<T>* p_parent = AT(p_addr, parent);
        RbtreeNode<T>* p_child = AT(p_addr, child);
    ...
    //        \                /
    //         d(R)           c(B)
    static void InsertFixUp(RbtreeHead* p_head, RbtreeNode<T>* p_addr, off_t z) {
        ...
        while ((p_parent = AT(p_addr, p_z->parent)) && 'R' == COLOR(p_parent)) {
            ...
            if (p_z->parent ==p_grandpa->left) {//父亲是爷爷的左孩子
                ...
                if ('R' == COLOR(p_uncle)) {    //case 1: 叔叔节点为红色
                    p_parent->color = 'B';
                    p_uncle->color = 'B';
                    p_grandpa->color = 'R';
                    p_z = p_grandpa;
                    z = p_parent->parent;
                } else if (z == p_parent->right) {   //case 2:z为父亲节点的右孩子 ,且叔叔节点为黑色
                    z = p_z->parent;
                    p_z = p_parent;                    
                    LeftRotate(p_head, p_addr, z);
                } else {                            //case 3：z为父亲节点的左孩子， 且叔叔节点为黑色
                    p_parent->color = 'B';
                    p_grandpa->color = 'R';
    ...
    // /\                                      /\
    // a b                                    b  r
    static bool RightRotate(RbtreeHead* p_head, RbtreeNode<T>* p_addr, off_t y) {
        ...
        if (-1 != p_head->free_stack) {//回收站非空
            ...
            if (p_head->size >= p_head->capacity) {//容量不足
                if (is_expand) {
                    ...
                    off_t new_capacity = p_head->capacity + add_size;
                    off_t len = sizeof(RbtreeHead) + sizeof(EXTEND) + new_capacity * sizeof(RbtreeNode<T>);
