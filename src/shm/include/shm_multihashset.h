#ifndef SHM_MULTIHASHSET_H_

#define SHM_MULTIHASHSET_H_
...
class ShmMultihashset {
public:
    ShmMultihashset(const char* name):p_head_(NULL), p_ext_(NULL), p_bucket_(NULL), p_addr_(NULL) {
        snprintf(name_, 255, name);
    }
    
    //创建共享内存
    bool CreateShm(off_t capacity) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::CreateShm(p_head_, p_ext_, p_bucket_, p_addr_, capacity, name_);
    }
    
    //加载共享内存
    bool AttachShm(bool is_readonly = false) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::AttachShm(p_head_, p_ext_, p_bucket_, p_addr_, name_, is_readonly);    
    }
    ...
    bool Insert(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::InsertMultiple(p_head_, p_bucket_, p_addr_, data);
    }
    
    //删除节点, 返回删除节点的个数
    off_t Remove(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::RemoveAll(p_head_, p_bucket_, p_addr_, data);
    }
    ...
    //升序遍历红黑树， 起始节点
    RbtreeNode<T>* Begin() {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::Begin(p_head_, p_bucket_, p_addr_);
    }
    
    //升序遍历红黑树， 下一个节点
    RbtreeNode<T>* Next(const RbtreeNode<T>* p_cur) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::Next(p_head_, p_bucket_, p_addr_, p_cur);        
    }
    ...
    //降序遍历红黑树， 起始节点
    RbtreeNode<T>* RBegin() {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::RBegin(p_head_, p_bucket_, p_addr_);
    }
    
    //降序遍历红黑树， 下一个节点
    RbtreeNode<T>* RNext(const RbtreeNode<T>* p_cur) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::RNext(p_head_, p_bucket_, p_addr_);
    }
    ...
    RbtreeNode<T>* EqualRange(const T& data) {
        return AT(p_addr_, (ShmRbtree<T, Compare>::EqualRange(p_addr_, p_bucket_[HashFunc(data) & (p_head_->bucket_size - 1)], data)));
    }
    ...
    const off_t Count(const T& data)const {
        return ShmRbtree<T, Compare>::Count(p_addr_, p_bucket_[HashFunc(data) & (p_head_->bucket_size - 1)], data);
    }    
