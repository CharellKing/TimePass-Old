...
class ShmHashset {
public:
    ShmHashset(const char* name):p_head_(NULL), p_ext_(NULL), p_bucket_(NULL), p_addr_(NULL) {
    ...
    bool Insert(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::InsertUnique(p_head_, p_bucket_, p_addr_, data);
    }
    
    //删除节点, 返回删除节点的个数
    off_t Remove(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::Remove(p_head_, p_bucket_, p_addr_, data);
    }
    ...
    RbtreeNode<T>* EqualRange(const T& data) {
        ...
        return AT(p_addr_, (ShmRbtree<T, Compare>::EqualRange(p_addr_, root, data)));
    }
