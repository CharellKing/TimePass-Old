...
class ShmList{
public:
    ShmList(const char* name):p_head_(NULL), p_addr_(NULL){
    ...
    //在链表头部弹出数据
    const ListNode<T>* PopFront() {
        return CommonList<T, EXTEND>::PopFront(p_addr_, p_head_);
    }
    
    //在链表尾部弹出数据
    const ListNode<T>* PopBack() {
        return CommonList<T, EXTEND>::PopBack(p_addr_, p_head_);
    }
