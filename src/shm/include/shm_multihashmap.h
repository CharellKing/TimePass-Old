#ifndef _SHM_MULTIHASHMAP_H

#define _SHM_MULTIHASHMAP_H

#include "shm_hashrbtree.h"

namespace TimePass{
template <typename KEY, typename VALUE, int (*Compare)(const KEY& a, const KEY& b) = KEY::Compare, off_t (*HashFunc)(const KEY& key) = KEY::HashFunc, typename EXTEND = char>
class ShmMultihashmap{
public:
    typedef ShmHashpair<KEY, VALUE, Compare, HashFunc> MAP_DATA;
    typedef RbtreeNode<MAP_DATA> MAP_NODE;
    typedef ShmHashrbtree<MAP_DATA, MAP_DATA::Compare, MAP_DATA::HashFunc, char> MAP_TREE;
    
    ShmMultihashmap(const char* name):p_head_(NULL), p_ext_(NULL), p_bucket_(NULL), p_addr_(NULL) {
        snprintf(name_, 255, name);
    }
    
    //创建共享内存
    bool CreateShm(off_t capacity) {
        return MAP_TREE::CreateShm(p_head_, p_ext_, p_bucket_, p_addr_, capacity, name_);
    }
    
    //加载共享内存
    bool AttachShm(bool is_readonly = false) {
        return MAP_TREE::AttachShm(p_head_, p_ext_, p_bucket_, p_addr_, name_, is_readonly);    
    }
    ...
    MAP_NODE* EqualRange(const MAP_DATA& data){
        return AT(p_addr_, (MAP_TREE::EqualRange(p_addr_, p_bucket_[HashFunc(data) & (p_head_->bucket_size - 1)], data)));
    }
    ...
    const off_t Count(const MAP_DATA& data)const {
        return MAP_TREE::Count(p_addr_, p_bucket_[HashFunc(data) & (p_head_->bucket_size - 1)], data);
    }
	...
	off_t TotalSize() const {
		return sizeof(HashrbtreeHead) + sizeof(EXTEND) + sizeof(Bucket) * p_head_->bucket_size + sizeof(MAP_NODE) * p_head_->capacity;	
	}
	
	off_t UsedSize() const {
		return sizeof(HashrbtreeHead) + sizeof(EXTEND) + sizeof(Bucket) * p_head_->bucket_size + sizeof(MAP_NODE) * p_head_->size;			
	}
