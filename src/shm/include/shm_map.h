...
namespace TimePass{
template <typename KEY, typename VALUE, int (*Compare)(const KEY& a, const KEY& b) = KEY::Compare, typename EXTEND = char>
class ShmMap{
public:
    ...
    ShmMap(const char* name):p_head_(NULL), p_addr_(NULL) {
        snprintf(name_, 255, name);
    }
    
    //创建共享内存
    bool CreateShm(off_t capacity) {
        p_head_ =  (RbtreeHead*)ShmBase::CreateShm(name_, capacity * sizeof(MAP_DATA) + sizeof(EXTEND) + sizeof(RbtreeHead));
        if (NULL == p_head_) {
            return false;
        }
        ...
        p_addr_ = (MAP_NODE*)((char*)p_head_ + sizeof(RbtreeHead) + sizeof(EXTEND));
        return true;
    }
    ...
    bool Insert(const MAP_DATA& data) {
        return MAP_TREE::InsertUnique(name_, p_head_, p_addr_, data);
    }
    
    //删除节点, 返回删除节点的个数
    off_t Remove(const KEY& key) {
        return MAP_TREE::Remove(p_head_, p_addr_, MAP_DATA(key));
    }
    ...
    MAP_NODE* LowerBound(const MAP_DATA& data) {
        return AT(p_addr_, (MAP_TREE::LowerBound(p_addr_, p_head_->root, data)));
    }
    ...
    MAP_NODE* UppperBound(const MAP_DATA& data) {
        return AT(p_addr_, (MAP_TREE::UpperBound(p_addr_, p_head_->root, data)));
    }
    ...
    MAP_NODE* EqualRange(const MAP_DATA& data) {
        return AT(p_addr_, (MAP_TREE::EqualRange(p_addr_, p_head_->root, data)));
    }
