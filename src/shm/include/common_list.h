...
struct ListHead {
    ListHead(off_t head, off_t tail, off_t capacity, off_t size, off_t free_stack): head(head), tail(tail),
		capacity(capacity), size(size), free_stack(free_stack) {
...
class CommonList {
public:
	//获取链表顺序为index的节点相对于起始地址的位置offset，以及节点指针 index [0, size)
	static off_t GetOffset(const ListNode<T>* p_addr, const ListHead* p_list_head, off_t index) {
    ...
    //在链表的尾部插入数据
    static ListNode<T>* PushBack(const char* name, ListNode<T>* p_addr, ListHead* p_list_head, const T& data, bool is_expand = true) {
		off_t free_node = -1;
		ListNode<T>* p_free_node = CommonList<T, EXTEND>::PopFreeNode(name, p_addr, p_list_head, free_node, is_expand);
		if (p_free_node) {
			...
			if (-1 == p_list_head->head) {
				p_list_head->head = free_node;
				p_list_head->tail = free_node;
			} else {
				ListNode<T>* p_tail = p_addr + p_list_head->tail;
				p_tail->next = free_node;
			#ifdef DOUBLY_LIST
	...
	//在链表的头部插入数据
	static ListNode<T>* PushFront(const char* name, ListNode<T>* p_addr, ListHead* p_list_head, const T& data, bool is_expand = true) {
		off_t free_node = -1;
		ListNode<T>* p_free_node = CommonList<T, EXTEND>::PopFreeNode(name, p_addr, p_list_head, free_node, is_expand);
		if (p_free_node) {
			p_free_node->data = data;
	...
	static const ListNode<T>* Insert(const char* name, ListNode<T>* p_addr, ListHead* p_list_head, const T& data, off_t index, bool is_expand = true) {
        if (index < 0 || index > p_list_head->size) {
    		Error::SetErrno(NO_INDEX_NONEXIST);
            Error::SetErrmsg(MSG_INDEX_NONEXIST);
            return NULL;            
        ...
        p_prior->next = free_node;
    #ifdef DOUBLY_LIST
        p_free_node->prior = p_next->prior;
        p_next->prior = free_node;
    #endif
	...
	//删除头结点
	static const ListNode<T>* PopFront(ListNode<T>* p_addr, ListHead* p_list_head) {
	...
	//删除尾节点
	static const ListNode<T>* PopBack(ListNode<T>* p_addr, ListHead* p_list_head) {
        if (p_list_head->size <= 0) {
            return NULL;
        }
        ...
        #else 
            p_list_head->tail = GetOffset(p_addr, p_list_head, p_list_head->size - 2);    
        #endif
	...
	//删除节点,index [0, size)
	static const ListNode<T>* Remove(ListNode<T>* p_addr, ListHead* p_list_head, off_t index) {
        if (index < 0 || index >= p_list_head->size) {
        	Error::SetErrno(NO_INDEX_NONEXIST);
            Error::SetErrmsg(MSG_INDEX_NONEXIST);
            return NULL;
    ...
    //优化链表
    static bool Optimize(const char* name, ListNode<T>* p_addr, ListHead* p_list_head){
        //获取缩减后的链表尺寸
        off_t new_capacity = ShmConfig::ReduceSize(p_list_head->capacity, p_list_head->size);
        ...
        if (p_list_head->head >= 0) {//先处理头部
            if (p_list_head->head >= new_capacity) {
                off_t new_head = FliterFreeNode(p_addr, p_list_head, new_capacity);
        ...
        //开始缩减
        off_t len = sizeof(ListHead) + sizeof(EXTEND) + sizeof(ListNode<T>) * new_capacity;
    ...
    static void DrawList(const ListNode<T>* p_addr, off_t head, std::ofstream& out, const std::string (*Label)(const T& value)) {
		std::string str;
        const ListNode<T> *p_cur = AT(p_addr, head);
		std::ostringstream oss;
		off_t prior = -1;
		off_t cur = head;
		if (-1 != head) {
			oss << "rankdir=LR;\n";
			oss << "Node" << head << "[shape=box, label=\"" << Label(p_cur->data) << "\"];\n";
			prior = cur;
			cur = p_cur->next;
            p_cur = AT(p_addr, cur);
		}
		...
		while(-1 != cur) {
			oss << "Node" << cur << "[shape=box, label=\"" << Label(p_cur->data) << "\"];\nNode" << prior << "->Node" << cur << "\n";
        #ifdef DOUBLY_LIST
			oss << "Node" << cur << "->Node" << prior << "\n";
        #endif
			prior = cur;
			cur = p_cur->next;
            p_cur = AT(p_addr, cur);
    ...
    static void Move(ListNode<T>* p_addr, off_t prior, off_t src, off_t dest) {
        const ListNode<T>* p_src = AT(p_addr, src);
        ListNode<T>* p_dest = AT(p_addr, dest);
    ...
    static off_t FliterFreeNode(ListNode<T>* p_addr, ListHead* p_list_head, off_t upper_limit) {
        off_t ret = -1; 
    ...
    static void FliterFreeStack(ListNode<T>* p_addr, ListHead* p_list_head,  off_t upper_limit) {
...
private:
    //从回收站里面压入一个节点
    static void PushFreeNode(ListNode<T>* p_addr, ListHead* p_list_head, off_t freenode) {
        if (-1 == freenode) {
            return ;
        }
        
        AT(p_addr, freenode)->next = p_list_head->free_stack;
        p_list_head->free_stack = freenode;
    }
    
	//从回收站里面弹出一个节点
	static ListNode<T>* PopFreeNode(const char* name, ListNode<T>* p_addr, ListHead* p_list_head, off_t& free_node, bool is_expand) {
		ListNode<T>* p_free_node = NULL;
        free_node = -1;
        if (-1 != p_list_head->free_stack) {//回收站不为空
            free_node = p_list_head->free_stack;
			p_free_node = p_addr + p_list_head->free_stack;
            p_list_head->free_stack = p_free_node->next;
        } else {
			if (p_list_head->size >= p_list_head->capacity) {// 没有了空余空间
				if (is_expand) {
	                off_t add_size = ShmConfig::ExpandSize(p_list_head->size);
	                if (add_size <= 0){
	                    return NULL;
	                }
	                off_t new_capacity = p_list_head->capacity + add_size;
	                off_t len = sizeof(ListHead) + sizeof(EXTEND) + new_capacity * sizeof(ListNode<T>);
			...
			free_node = p_list_head->size;
			p_free_node = p_addr + free_node;
        }
        return p_free_node;
    }
};
